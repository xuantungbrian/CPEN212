...
void conv2(const double *wts, const double *bias, const double *iacts, double *oacts, size_t batch_sz) {
	...
	for (size_t N = 0; N < batch_sz; N += 50) {
		for (size_t K = 0; K < CONV2_K; K += 32) {
			for (size_t C = 0; C < CONV2_C; C += 2) {
				for (size_t Y = 0; Y < IN3_Y; Y += 2) {
					for (size_t n = N; (n < N + 50) && n < batch_sz; ++n) {
						for (size_t k = K; (k < K + 32) && k < CONV2_K; ++k) {
							for (size_t c = C; (c < C + 2) && c < CONV2_C; ++c) {
								for (size_t y = Y; (y < Y + 2) && y < IN3_Y; ++y) {
